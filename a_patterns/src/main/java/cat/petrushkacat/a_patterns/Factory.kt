...
class Factory {
    ...
    fun createDelivery(country: Country): Delivery {
        return when(country) {
            is Country.Russia -> {
                Delivery.Rail(country.companyName, country.weight, country.size, country.railDeliveryType)
            }
            is Country.Belarus -> {
                Delivery.Auto(country.companyName, country.weight, country.size)
            }
            is Country.USA -> {
                Delivery.Water(country.companyName, country.weight, country.size, country.waterDeliveryType)
            }
            is Country.Germany -> {
